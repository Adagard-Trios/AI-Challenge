...
| Component | URL | Notes |
|-----------|-----|-------|
| **Dashboard** | https://roger.nivakaran.dev | Frontend on Render. Sign-in required, the dashboard is not public |
| **API** | https://api.nivakaran.dev | Served from the maintainer's machine over a Cloudflare tunnel, see [HOSTING.md](HOSTING.md) |
| **Health + configuration** | https://api.nivakaran.dev/api/status | Reports which features are live and which are not, by name |

**The API runs on a laptop, not in a datacentre.** That is a deliberate trade
and the honest caveat with it is that the link is live only while that machine
is awake and online. A free 512 MB instance cannot hold TensorFlow alongside the
API, so hosting it here is what lets the weather LSTM and the currency GRU run
at all rather than reporting themselves unavailable.

Three of the four ML models work: weather (LSTM, 25 districts), currency (GRU,
next-day return) and anomaly detection. **Stock prediction does not**, and it is
not a training problem: Yahoo Finance carries no Colombo Stock Exchange listings
in any symbol format, so the model has no data to learn from. That card reports
itself unavailable rather than showing numbers nothing stands behind.

Several source scrapers are in the same position and say so on the card:
CEYPETCO is unreachable, and waterboard.lk and news.lk sit behind a bot
challenge this project does not attempt to defeat. Every panel carries a
provenance badge (`LIVE` / `NOT LIVE` / `NO SOURCE` / `FETCH FAILED`) so what is
fresh and what is not is visible without reading the code.
