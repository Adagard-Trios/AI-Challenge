...
# The only role. Named rather than inlined so "is there still a second tier?"
# has one answer to read, and so the string that is_admin compares against and
# the string new accounts are written with cannot drift apart.
DEFAULT_ROLE = "admin"
...
class User(Base):
    ...
    # One role. The viewer/admin split existed only to keep self-registered
    # accounts away from a machine-global credential vault; that vault is now
    # per-user, so the tier was gating nothing and merely stopped people
    # connecting their own accounts. The column stays -- dropping it buys
    # nothing and costs a migration on a live database -- but every account
    # written from here on gets DEFAULT_ROLE, and is_admin is true for all of
    # them. Existing "viewer" rows are lifted by _normalise_roles at startup.
