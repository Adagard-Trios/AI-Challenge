...
from .models import (
    DEFAULT_ROLE, ConnectorDevice, IngestedPost, Invite, SocialConnection, User,
    utcnow,
)
...
def register(payload: RegisterRequest, db: Session = Depends(get_db)):
    """
    Create an account. There is one kind of account.

    This used to mint a "viewer", a deliberately powerless role, because the
    social vault and session store were machine-global: one CredentialVault for
    the whole install, so any account that could reach /api/social/* could list
    the OWNER'S connected accounts and collect using their session. Open
    registration plus a real role would have handed a stranger the owner's
    Instagram.

    That shared state is gone. SocialService is constructed per user and seals
    each one's credentials, cookies and pacing under their own directory with
    their own key, so a new account starts empty and can only ever reach what it
    connected itself. With nothing left to escalate into, a second tier was
    protecting nothing and only stopped people using their own accounts.
    """
    ...
    pair = issue_pair(db, user)
    db.commit()
    logger.info("[auth] self-registered %s", email)
    return {**pair.as_dict(), "user": _user_out(user)}
