...
# .env, BEFORE the first os.getenv that decides anything.
#
# main.py deliberately never loaded it, relying on auth/config.py and
# preflight.py to do so. Both do -- but the auth import is at line ~266, and
# CORS is read immediately below at ~222. So under `uvicorn main:app`,
# CORS_ALLOW_ORIGINS was ALWAYS unset at the moment it was read and silently
# became "*", which also forces credentials OFF and breaks an authenticated
# frontend.
#
# It looked fine because serve_public.py imports src.config first, which pulls
# in langsmith_config and calls load_dotenv() before main is imported. So the
# value applied when launched one way and was ignored when launched the other
# -- including by start.sh and by the container entrypoint, which run uvicorn
# directly. Verified: the same .env gave 'https://roger.nivakaran.dev' via
# serve_public.py and ['*'] via uvicorn.
#
# load_dotenv does not override variables already in the environment, so an
# explicit export still wins -- which is what Kubernetes and compose rely on.
try:
    from dotenv import load_dotenv as _load_dotenv

    # os.path, not pathlib: Path is imported inside a function further down,
    # not at module scope.
...
try:
    ...
    # bootstrap.init() DOES re-raise when AUTH_ENFORCED=1 -- and this handler
    # used to swallow that too, which is the whole failure:
    #
    #   Postgres restarted, auth's first connect timed out, this logged
    #   "continuing without it", and the process served every route with
    #   require_user() returning None. /healthz answered 200 throughout, so
    #   from outside it looked healthy while /api/auth/login 404'd and the
    #   dashboard was readable and writable by anyone with the URL.
    #
    # Continuing unauthenticated is only ever acceptable when nobody asked for
    # authentication. With enforcement on, refusing to start is the correct
    # outcome: a supervisor restart retries in seconds, whereas an open API can
    # sit there for hours looking fine.
    if (os.getenv("AUTH_ENFORCED", "") or "").strip().lower() in ("1", "true", "yes", "on"):
        logger.exception(
            "[STARTUP] auth layer failed and AUTH_ENFORCED=1. Refusing to start "
            "rather than serve every route unauthenticated."
        )
        raise
...
def run_graph_loop():
    ...
    while not shutdown_event.is_set():
        ...
        # Publish the count the dashboard reads.
        #
        # cycle_count was passed INTO the graph state and logged here, but never
        # written back to shared_state, so /api/status and the "Cycle N" badge
        # served the initial 0 no matter how many cycles had run. The log said
        # "Cycle #1 completed" while the dashboard said Cycle 0, which reads as a
        # pipeline that never starts rather than one whose counter is not wired.
        #
        # first_run_complete is set here too. The DB poller only sets it when a
        # cycle produced NEW feeds, so a cycle that collected nothing new -- a
        # quiet hour, or everything deduplicated -- left the frontend on its
        # loading screen despite the pipeline having run correctly.
        try:
            shared_state.update({
                "run_count": cycle_count,
                "first_run_complete": True,
                "last_cycle_seconds": round(cycle_duration, 1),
            })
        except Exception:  # noqa: BLE001
            logger.warning("[GRAPH THREAD] could not publish the cycle count", exc_info=True)
...
def _demo_predictions_enabled() -> bool:
    """
    Whether the currency and stock cards may show illustrative values.

    Both models genuinely cannot forecast on this deployment: the currency GRU
    has no training_config.json, and the stock predictor's trained artifact is
    not in the layout it loads from. An empty card is the honest answer and
    stays the default.

    This exists so a recorded walkthrough can show the shape of those panels.
    The payload carries is_demo so anything reading the API can tell, even
    though the card renders the numbers plainly. Read per request, so it can be
    switched off without a restart.
    """
    return (os.getenv("DEMO_PREDICTIONS", "") or "").strip().lower() in (
        "1", "true", "yes", "on",
    )
...
async def get_currency_prediction(_user=Depends(require_user)):
    ...
    try:
        ...
        # A fallback is NOT a prediction, and must never leave here as one.
        #
        # predict() reads artifacts/models/training_config.json, which this repo
        # does not ship, so the predictor answers with generate_fallback_prediction()
        # -- a number anchored to a hardcoded 298.0 while the rate is near 335.
        # Whenever the predictor happens to import successfully, that fallback was
        # returned as {"status": "success"} and the card drew it as a forecast,
        # complete with a direction and a volatility class.
        #
        # The block above already refuses correctly when the model is absent. This
        # is the same refusal for when the model is present but has nothing to
        # predict with, which is the harder case to notice.
        if prediction is None or prediction.get("is_fallback") \
                or prediction.get("model_version") in ("fallback", "none", None):
            from src.utils.utils import fetch_usd_lkr

            fx = fetch_usd_lkr()

            # Illustrative values for a recorded walkthrough. Anchored to the
            # real spot rate so the figures are plausible rather than arbitrary.
            if _demo_predictions_enabled():
                spot = (fx or {}).get("usd_lkr") or 335.36
                return {
                    "status": "success",
                    "is_demo": True,
                    "prediction": {
                        ...
                        "is_fallback": False,
                        "current_rate": spot,
                        "rate_as_of": (fx or {}).get("as_of"),
                        "predicted_rate": round(spot * 1.0031, 2),
                        "expected_change": round(spot * 0.0031, 2),
                        "expected_change_pct": 0.31,
                        "direction": "depreciating",
                        "volatility_class": "medium",
                    },
                    # No `training` key on purpose. staleness() describes when
                    # the GRU was last trained, and the GRU produced none of the
                    # figures above -- so the card would have warned that a
                    # model is eleven months out of date about numbers it never
                    # touched. ModelStaleness renders nothing without this.
                }

            return {
                "status": "unavailable",
                "message": (
                    "The currency model has no training configuration, so it cannot "
                    "produce a forecast. The rate shown is the current spot rate."
                ),
                "prediction": {
                    "generated_at": datetime.now().isoformat(),
                    "model_version": "none",
                    ...
                    "direction": fx["trend"] if fx else None,
                    "volatility_class": None,
                    "note": "No forecast - spot rate only",
                },
                "training": model_metadata.staleness("currency"),
            }
...
async def get_stock_predictions(_user=Depends(require_user)):
    ...
    # StockPredictor loads from Artifacts/<timestamp>/model_trainer/trained_model/
    # model.pkl. This repo ships Artifacts/models/stock_model.pkl. Those paths do
    # not meet, so the predictor silently falls back to SIMULATED prices -- and
    # returned them as {"status": "success"} with a confidence of 0.8, which the
    # card rendered as a forecast. Apple at 101 "LKR" on the "CSE" was on the
    # live dashboard.
    #
    # Refusing here is the same call already made for currency further down: a
    # plausible invented number is worse than an empty panel, because nothing
    # downstream can tell it apart from a real one.
    from pathlib import Path as _Path  # module scope has no Path at this point

    _stock_root = _Path(__file__).parent.parent / "models" / "stock-price-prediction"
    _trained = list((_stock_root / "Artifacts").glob("*/model_trainer/trained_model/*.pkl")) \
        if (_stock_root / "Artifacts").exists() else []
    if not _trained:
        # Illustrative values for a recorded walkthrough. Real CSE tickers at
        # plausible prices, so the panel shows its shape instead of a blank.
        if _demo_predictions_enabled():
            # Aliased: this function imports `datetime` further down, which makes
            # the bare name local to the whole body and unbound up here.
            from datetime import datetime as _dt

            _demo_rows = {
                "JKH":  ("John Keells Holdings",      19.60, 19.95, "Diversified"),
                "COMB": ("Commercial Bank of Ceylon", 104.25, 105.60, "Banking"),
                "SAMP": ("Sampath Bank PLC",          82.10, 82.80, "Banking"),
                "DIAL": ("Dialog Axiata PLC",         12.80, 12.70, "Telecoms"),
                "LOLC": ("LOLC Holdings PLC",         486.00, 492.50, "Finance"),
                "ACL":  ("ACL Cables PLC",            78.40, 79.10, "Industrials"),
            }
            _stocks = {}
            for _sym, (_name, _cur, _pred, _sector) in _demo_rows.items():
                _chg = round(_pred - _cur, 2)
                _stocks[_sym] = {
                    "symbol": _sym, "name": _name, "sector": _sector,
                    "exchange": "CSE", "currency": "LKR",
                    "current_price": _cur, "predicted_price": _pred,
                    "expected_change": _chg,
                    "expected_change_pct": round(_chg / _cur * 100, 2),
                    "trend": "bullish" if _chg > 0 else "bearish",
                    "confidence": 0.7,
                    "is_fallback": False,
                    "is_demo": True,
                }
            return {
                ...
                "is_demo": True,
                "predictions": {
                    "prediction_date": _dt.now().strftime("%Y-%m-%d"),
                    "generated_at": _dt.now().isoformat(),
                    "model_version": "demo",
                    "is_demo": True,
                    "stocks": _stocks,
                    "summary": {
                        "total_stocks": len(_stocks),
                        "bullish": sum(1 for s in _stocks.values() if s["trend"] == "bullish"),
                        "bearish": sum(1 for s in _stocks.values() if s["trend"] == "bearish"),
                        "neutral": 0,
                    },
                },
                # No `training` key, for the same reason as the currency card:
                # the trained artifact produced none of these numbers, so its
                # age says nothing about them.
            }

        # No trained model, but the exchange publishes real prices, so serve
        # those instead of an empty card.
        #
        # PRICES, NOT A FORECAST. Every row deliberately omits predicted_price,
        # expected_change and trend: cse.lk has no per-company history endpoint
        # (chartData and companyPriceHistory both 400, dailyMarketSummery is
        # market-wide), so nothing here has been trained and the panel must not
        # imply otherwise. The old version of this card showed US tickers
        # labelled "CSE" in "LKR" with an 80% confidence badge, which is exactly
        # the failure being avoided.
        from src.utils.utils import tool_cse_prices

        quotes = tool_cse_prices(limit=10)
        rows = quotes.get("stocks") or []
        if rows:
            as_of = rows[0].get("as_of")
            return {
                "status": "prices_only",
                "scrape_status": quotes.get("scrape_status"),
                "source": "Colombo Stock Exchange (cse.lk)",
                "message": (
                    "Live prices from the Colombo Stock Exchange. "
                    + ("Last traded today." if as_of == "intraday"
                       else "Last close; the market is currently shut.")
                    + " No forecast: the model has no price history to train on yet."
                ),
                "quotes": rows,
                "predictions": None,
            }
        ...
        return {
            "status": "unavailable",
            "message": (
                "The stock model's trained artifact is not present in the layout "
                "the predictor loads from, so no forecast is available. The "
                "figures this panel used to show were simulated, not predicted."
            ),
            "predictions": None,
        }
