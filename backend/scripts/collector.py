...
def collector_user_id() -> "str | None":
    """
    Which user this collector runs as.

    It matters now that it did not before. Sessions and credentials used to live
    in one machine-global store, so reading them needed no identity; they are
    sealed per user today, and get_service(None) opens the OLD shared directory.
    A collector left unscoped would therefore report "no connected accounts" on
    a machine where accounts are plainly connected -- the failure looks like a
    quiet day rather than a lookup against the wrong store.

    The earliest-created account, which is the one the machine owner made. Posts
    are attributed to the same user in _persist, so what is collected and who it
    ...
    try:
        ...
        with session_scope() as session:
            owner = session.query(User).order_by(User.created_at.asc()).first()
            _owner_id_cache = owner.id if owner else None
    except Exception as exc:  # noqa: BLE001
        logger.warning("could not resolve the collector's user: %s", exc)
...
def describe_environment() -> dict:
    ...
    try:
        ...
        facts["connected"] = ", ".join(
            a["platform"] for a in (get_service(collector_user_id()).accounts() or [])
...
def _persist(posts: list, platform: str) -> int:
    ...
    try:
        with session_scope() as session:
            # The SAME user whose session store was read, resolved once in
            # collector_user_id(). Selecting the owner independently here is how
            # these two drift: with per-user vaults, collecting from one user's
            # cookies and filing the results under another's id would show them
            # on a dashboard that never connected the account.
            owner_id = collector_user_id()
            owner = (
                session.get(User, owner_id) if owner_id else None
...
def collect_once() -> dict:
    ...
    results: dict = {}
    try:
        accounts = get_service(collector_user_id()).accounts() or []
    except Exception as exc:  # noqa: BLE001
