...
def main() -> int:
    ...
    # Import the app HERE, before uvicorn does, so a broken auth layer is
    # discovered by this process rather than inside a worker.
    #
    # The checks above read environment variables. They cannot tell whether auth
    # actually came up, and the difference is not academic: Postgres restarted
    # once during development, auth's first connect timed out, main.py logged
    # "continuing without it", and the process served every route with
    # require_user() returning None -- while /healthz answered 200 and these
    # same checks reported "All checks passed", because AUTH_ENFORCED really was
    # 1. start-backend.sh gates the tunnel on the log line for this reason; a
    # backend started directly skipped that entirely.
    if _enforcement_requested():
        try:
            import main as _app_module
        except Exception:
            print("\n  REFUSING TO SERVE: the application failed to import.\n")
            raise

        if not getattr(_app_module, "_AUTH_READY", False):
            print(
                "\n  REFUSING TO SERVE.\n\n"
                "  AUTH_ENFORCED is on but the auth layer did not come up, so\n"
                "  every route would be readable and writable by anyone with\n"
                "  the URL. Check the database is reachable and try again.\n"
            )
            return 1
...
def _enforcement_requested() -> bool:
    import os

    return (os.getenv("AUTH_ENFORCED", "") or "").strip().lower() in (
        "1", "true", "yes", "on",
    )
