...
def _last_run_ages() -> Dict[str, float]:
    ...
    IN SHADOW MODE, COUNT PLANNED ROWS TOO.
    -----------------------------------------
    This filtered on executed=True unconditionally, and shadow mode executes
    nothing. So the map came back empty on every tick, age_of() returned its
    "never ran" sentinel for every source, is_starving() was therefore always
    true, and the reason string -- "overdue" if starving else the focus term --
    could only ever say "overdue".

    Measured on the real ledger: 617 activations across 53 ticks and two days,
    every single one trigger_reason='overdue'. Not one recorded a focus, while
    live foci sat at urgency 0.66 and a collector was watching that exact kind.

    The instrumentation could not produce the evidence it exists to produce.
    In shadow the fan-out is doing the collecting on its own schedule, so the
    planner's clock should advance when it PLANS a source; that makes
    starvation meaningful and lets the opportunistic reasons appear. In active
    mode this stays strictly executed=True, because there a planned-but-unrun
    source genuinely has not run.
    """
    try:
        ...
        with session_scope() as session:
            query = session.query(KSActivation)
            if mode() != "shadow":
                query = query.filter(KSActivation.executed.is_(True))
            rows = (
                query
        ...
        # An empty map means "nothing has ever run" to is_starving(), so every
        # source is promoted and the agenda degrades to "run everything" --
        # which is exactly the state the ledger spent two days recording
        # without anyone being able to see why.
        logger.warning("[controller] could not read run ages; every source "
                       "will look starved this tick", exc_info=True)
        return {}
