...
def _run_competitor_mentions(params):
    """
    Search the topics and stories currently in focus for the competitors a user
    named, through the same registry the agent tools use so it inherits the
    pacing gate and the daily budget.

    Previously run=None: the source sat on the agenda, was recorded, and would
    have been a no-op had the controller ever claimed it.
    """
    from src.scrapers import registry

    watchlist = _competitor_watchlist()
    if not watchlist:
        # No competitor named by anyone. Not a failure -- there is genuinely
        # nothing to look for, and saying so beats scraping "sri lanka".
        return {"status": "no_watchlist"}

    subjects = (params.get("topic") or []) + (params.get("story") or [])
    results = {}
    for name in watchlist[:2]:
        query = f"{name} {subjects[0]}" if subjects else str(name)
        outcome = registry.run("scrape_twitter", query, max_items=10)
        results[str(name)] = outcome.get("status")
    return results


def _competitor_watchlist() -> List[str]:
    """
    Competitor handles from src/config/intel_config.json.

    ExposureProfile has districts, infrastructure, lanes and sectors, but no
    competitors column, so the configured defaults are the only watchlist that
    exists. Reading the file the intelligence agent already reads keeps one
    source of truth rather than inventing a second.
    """
    try:
        import json
        from pathlib import Path

        path = Path(__file__).resolve().parent.parent / "config" / "intel_config.json"
        data = json.loads(path.read_text(encoding="utf-8"))

        names: List[str] = []
        defaults = data.get("default_competitors") or {}
        for sector in defaults.values():
            if not isinstance(sector, dict):
                continue
            for handles in sector.values():
                for value in (handles or []):
                    text = str(value).strip()
                    if text and text not in names:
                        names.append(text)
        return names
    except Exception:  # noqa: BLE001
        # Missing or malformed config. An empty watchlist is the correct
        # answer, not an exception that kills the whole agenda -- but it is
        # also indistinguishable from "nobody named a competitor", so say so.
        logger.warning("[ks] competitor config unreadable; watchlist is empty",
                       exc_info=True)
        return []


def _summarise_domain(domain: str):
    """
    Build the run() for one domain's summariser.

    Goes through the domain agent's own summary path rather than calling the
    LLM here, so the wording, the truncation limit and the salvage parser stay
    in one place. A second copy would drift, and the copy that drifted would be
    the one nobody was reading.
    """
    def run(_params):
        from .store import BoardStore

        entries = BoardStore().top_entries(limit=25, domain=domain)
        if not entries:
            return {"status": "nothing_to_summarise", "domain": domain}

        from src.intelligence.summaries import AGENT_SUMMARY_LIMIT, truncate
        from src.llms.groqllm import GroqLLM

        joined = "\n".join(
            f"- {str(e.get('summary') or e.get('content') or '')[:200]}"
            for e in entries[:25]
        )
        prompt = (
            f"Summarise the {domain} situation in Sri Lanka from these "
            f"observations. Be specific and factual. Do not use Markdown.\n\n"
            f"{joined}"
        )
        text = GroqLLM().get_llm().invoke(prompt)
        content = getattr(text, "content", "") or ""
        return {
            "domain": domain,
            "entries": len(entries),
            "summary": truncate(content, AGENT_SUMMARY_LIMIT),
        }
...
def _collector(name, domain, *, max_minutes, focus_kinds=(), tokens=0,
    ...
    def trigger(digest: BoardDigest) -> Optional[Activation]:
        ...
        # Report BOTH signals when both apply.
        #
        # This was "overdue" if starving else the focus term, so starvation
        # silently swallowed the focus whenever they coincided -- and they
        # coincide often, because a source that has waited long enough to be
        # overdue is exactly the one a hot focus is most likely to want. The
        # ledger then showed a run as purely scheduled when opportunistic
        # control had in fact selected it, which is the opposite of what the
        # ledger is for.
        if starving and urgency >= 0.5:
            reason = f"overdue; focus urgency {urgency:.2f}"
        elif starving:
            reason = "overdue"
        else:
            reason = f"focus urgency {urgency:.2f}"
        params = {}
...
def _summariser(name, domain, *, max_minutes, tokens, description="", run=None):
    """
    ...
    def trigger(digest: BoardDigest) -> Optional[Activation]:
        ...
        return Activation(
            ...
            # Both signals, for the reason given on the collector trigger
            # above: starvation was hiding severity whenever they coincided,
            # so the ledger could not show that severity had selected a run.
            (f"overdue; new {domain} severity {severity:.2f}"
             if starving and severity >= 0.5
             else "overdue" if starving
             else f"new {domain} severity {severity:.2f}"),
            {},
            tokens,
        )
...
_register(_collector(
    "econ.local_news", "economical", max_minutes=45,
    focus_kinds=("story", "topic"), run=_run_local_news,
    description="Five Sri Lankan outlets, round-robin. Watches story and topic "
                "foci: a developing story is exactly what the outlets follow up, "
                "and those are the two kinds the sensors actually produce."))
_register(_collector(
...
_register(_collector(
    ...
    # Was focus_kinds=("entity",). No sensor has ever written an "entity"
    # focus -- the only kinds produced are district, story and topic -- so this
    # watched a kind that does not exist and its urgency was permanently 0.
    focus_kinds=("topic", "story"), run=_run_competitor_mentions,
    description="Named competitors, followed through the topic and story foci "
...
for _domain in ("social", "political", "economical", "meteorological",
    ...
    _register(_summariser(
        ...
        run=_summarise_domain(_domain),
        description=f"LLM summary for {_domain}, over that domain's recent "
                    f"board entries."))
