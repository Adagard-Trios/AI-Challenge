...
def _tend_the_board(snapshot: Dict[str, Any]) -> None:
    """
    Mirror the assessment, refresh the foci, plan, then age and evict.

    One function called from every exit of the refresher node, rather than a
    tail that two early returns skipped. Individually guarded: a board that
    cannot be written must not take the cycle down with it, and the node's
    return value does not depend on any of this succeeding.
    """
    for step, fn in (
        ("assessment", lambda: _shadow_write_assessment(snapshot)),
        ("foci", lambda: _write_foci(snapshot)),
        ("controller", _run_shadow_controller),
        ("decay", _run_decay_pass),
    ):
        try:
            fn()
        except Exception as exc:  # noqa: BLE001
            logger.warning("[Blackboard] %s step failed: %s", step, exc)
...
JSON array only:"""
    ...
    def data_refresher_agent(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not feed:
            ...
            # The board still has to be tended on a quiet cycle.
            #
            # Both of these returns used to jump straight past the controller
            # tick and the decay pass at the end of this function, so on any
            # cycle that produced no new events the blackboard simply stopped
            # observing. That is exactly backwards: a quiet feed is when
            # starvation matters most, because it is the signal that a source
            # is overdue and should be scheduled. Measured before this: 24
            # activations recorded in three hours of a running backend, all of
            # them from before the last restart.
            _tend_the_board(previous or snapshot)
