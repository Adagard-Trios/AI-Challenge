...
def _shared():
    ...
    try:
        ...
        # not. Without Redis there is no shared state, so
        # each replica serves its own dashboard snapshot -- and the only
        ...
        if not _WARNED_NO_REDIS:
            ...
            logger.warning(
                "[shared_state] Redis unavailable; using per-process fallback. "
                "With more than one replica, each replica serves its own dashboard snapshot.",
                exc_info=True,
            )
        return None
