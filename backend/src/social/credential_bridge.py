...
class SessionStoreCredentialStore:
    ...
    def available(self) -> List[str]:
        try:
            ...
            # [] here means "no connected accounts" to every caller, and the
            # agents read exactly this to decide what to scrape. A session
            # store that cannot be opened would therefore produce a cycle that
            # collects nothing and reports success -- the failure mode this
            # project keeps rediscovering. The value stays safe; the silence
            # does not.
            logger.warning("[bridge] session store unreadable; reporting no "
                           "connected accounts", exc_info=True)
            return []
