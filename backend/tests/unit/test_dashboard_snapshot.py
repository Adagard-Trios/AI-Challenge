...
def test_the_cycle_count_reaches_shared_state():
    """
    run_count was passed into the graph state and logged, but never written
    back, so /api/status and the "Cycle N" badge served the initial 0 forever.
    The log read "Cycle #1 completed" while the dashboard read Cycle 0, which
    looks like a pipeline that never starts rather than an unwired counter.
    """
    import ast
    from pathlib import Path

    source = (Path(__file__).parent.parent.parent / "main.py").read_text(encoding="utf-8")
    fn = next(
        n for n in ast.walk(ast.parse(source))
        if isinstance(n, (ast.FunctionDef, ast.AsyncFunctionDef))
        and "graph_loop" in n.name
    )
    body = ast.get_source_segment(source, fn) or ""

    assert '"run_count": cycle_count' in body, (
        "the graph loop never publishes cycle_count, so the dashboard cannot "
        "show how many cycles have run"
    )
    assert '"first_run_complete": True' in body, (
        "first_run_complete is only set by the DB poller, which runs when a "
        "cycle produced NEW feeds -- so a quiet cycle leaves the frontend on "
        "its loading screen"
    )
