...
def test_two_users_get_separate_vaults_sessions_and_jobs():
    """
    The property the removed admin check used to provide.

    Not a check that the argument is passed -- that is elsewhere -- but that
    passing it actually separates the stores. If _store_dir ever returned the
    same path for two ids, every route would still look correct while both
    users shared one Instagram slot.
    """
    from src.social.service import SocialService

    a = SocialService(user_id="user-aaaa")
    b = SocialService(user_id="user-bbbb")
    shared = SocialService(user_id=None)

    assert a._store_dir() != b._store_dir(), "two users resolve to one directory"
    assert a._store_dir() != shared._store_dir(), (
        "a scoped user lands in the install-wide directory"
    )
    # The raw id must not be reconstructable from the path.
    assert "user-aaaa" not in str(a._store_dir())
    # Jobs are per-instance, so an in-flight login cannot be observed or
    # cancelled by another account.
    assert a._jobs is not b._jobs


def test_the_unscoped_service_is_not_reused_for_a_named_user():
    """get_service caches per id; a cache keyed too loosely would hand user B
    the instance -- and therefore the open vault -- built for user A."""
    from src.social import service as mod

    mod.reset_service()
    try:
        first = mod.get_service("user-aaaa")
        assert mod.get_service("user-aaaa") is first, "not cached per user"
        assert mod.get_service("user-bbbb") is not first, "cache collides across users"
        assert mod.get_service() is not first, "unscoped call returns a user's service"
    finally:
        mod.reset_service()
