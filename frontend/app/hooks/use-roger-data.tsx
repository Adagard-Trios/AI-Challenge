...
function useRogerDataInternal() {
  // Only for "has the signed-in user changed" -- the situational fetches below
  // are useless until there is a token, and this provider mounts before login.
  const { user } = useAuth();
  ...
  // Distinct from "not connected": the server actively refused us because we
  // are signed out. main.py closes with 1008 specifically so this is
  // distinguishable -- without it a signed-out visitor is told "Reconnecting"
  // forever, which blames the network for what is really "please log in".
    ...
    const connect = async () => {
      try {
        ...
        websocket.onclose = (event) => {
          ...
          // 1008 = policy violation, which the backend uses only for "no valid
          // ticket". Anything else is a genuine transport problem, so clear the
          // flag rather than leaving a stale "sign in" prompt on screen after
          // the user has signed in and the socket drops for an unrelated reason.
          setAuthRequired(event.code === 1008);
  ...
  //
  // A FAILED REQUEST IS NOT AN ABSENT SOURCE. These used to be
  // `if (res?.ok) setX(...)` with no else, so a 401, a timeout or a dropped
  // connection left the state at null -- and null renders as "NO SOURCE",
  // whose tooltip reads "There is no readable source for this data yet."
  // That is a claim about CEB and CBSL, made because our own request failed.
  //
  // On failure the panel is now given {scrape_status: "error"}, which the
  // provenance badge already renders as FETCH FAILED. Same machinery, but the
  // card stops blaming the source for our problem.
  const fetchSituationalData = useCallback(async (): Promise<boolean> => {
    let failed = false;
    const sources: Array<[string, (v: Record<string, unknown> | null) => void]> = [
      ['power', setPowerData],
      ['fuel', setFuelData],
      ['economy', setEconomyData],
      ['health', setHealthData],
      ['commodities', setCommodityData],
      ['water', setWaterData],
    ];

    await Promise.all(sources.map(async ([path, setter]) => {
      try {
        ...
        // A 401 before sign-in is expected and transient: the effect reruns on
        // user change. Marking it FETCH FAILED would flash a red badge at
        // every visitor on the login screen.
        if (res.status !== 401) {
          setter({ scrape_status: 'error', http_status: res.status });
          failed = true;
        }
      } catch (err) {
        console.warn(`[Roger] /api/${path} failed:`, err);
        setter({ scrape_status: 'error' });
        failed = true;
      }
    }));
  ...
  // Fetch situational data periodically (every 5 minutes), AND whenever the
  // signed-in user changes.
  //
  // The user dependency is what makes these panels fill in. This provider is
  // mounted in ClientWrapper, above the auth gate, so it runs once while the
  // visitor is still signed out: all six requests 401, `if (res?.ok)` keeps the
  // state at null, and nothing retries for five minutes. Signing in does not
  // remount the provider, so power, fuel, economy, health, commodities and
  // water all sat empty on a dashboard whose API was answering perfectly --
  // reported as "NO SOURCE", which reads as a dead scraper rather than a
  // request made a moment too early.
  //
  // RETRY SOON AFTER A FAILURE, not in five minutes. The interval alone means a
  // backend restart, a dropped connection or a cold start leaves six panels
  // reading NO SOURCE for the rest of the window, and that is not theoretical:
  // it is what this dashboard showed every time the API was restarted during
  // development. A short backoff makes a transient failure self-heal instead of
  // persisting long enough for someone to conclude the sources are dead.
  useEffect(() => {
    let cancelled = false;
    let timer: ReturnType<typeof setTimeout>;
    let attempt = 0;

    const HEALTHY_MS = 300_000;              // 5 min when everything answered
    const RETRY_MS = [15_000, 30_000, 60_000];  // then back to the slow loop

    const run = async () => {
      const failed = await fetchSituationalData();
      if (cancelled) return;
      attempt = failed ? Math.min(attempt + 1, RETRY_MS.length) : 0;
      const wait = attempt === 0 ? HEALTHY_MS : RETRY_MS[attempt - 1];
