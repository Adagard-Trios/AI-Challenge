...
/**
 * GET that tells you whether it worked.
 *
 * Prefer this to apiGet for anything a panel renders as a list. apiGet returns
 * the fallback on failure, so `{ stories: [] }` means BOTH "the pipeline has
 * produced no stories" and "the request died", and the card draws the same
 * empty state either way. That has now bitten this project four separate times:
 * the social-accounts panel, the registration probe, the six situational cards
 * and the intelligence feed. Every one looked like "there is nothing here"
 * while the truth was "we could not ask".
 *
 * apiGet stays for genuinely optional extras, where an absent value and a
 * failed lookup really are the same to the reader.
 */
...
export async function apiResult<T>(path: string): Promise<ApiResult<T>> {
  try {
    ...
    if (err instanceof ApiError) {
      return { ok: false, error: err.message || `Request failed (${err.status})`,
               status: err.status };
    }
 ...
 *
 * There is one kind of account. Self-registration used to mint a powerless
 * `viewer`, because the social credential vault was machine-global and any real
 * account reached the owner's connected Instagram. The vault is keyed per user
 * now -- own directory, own encryption key -- so a new account starts empty and
 * can only reach what it connects itself.
 */
...
/**
 * null = could not ask (offline, CORS, backend down) -- deliberately NOT false.
 *
 * apiGet's swallow-everything fallback made an unreachable server look exactly
 * like a server that answered "sign-ups are closed", so the login page told
 * people registration was invite-only whenever the probe failed. The caller
 * needs to tell those apart to avoid stating something untrue.
 */
export async function registrationOpen(): Promise<boolean | null> {
  try {
    const data = await api<{ open: boolean }>("/api/auth/registration");
