...
class CurrencyGRUTrainer:
    ...
    def prepare_data(
        ...
        #
        # The target is the NEXT-DAY RETURN, not the closing level.
        #
        # Predicting the level could not work here and the failure was not
        # subtle: USD/LKR trends upward, MinMaxScaler is fitted on the training
        # window, and a GRU with a bounded output cannot emit a value above the
        # maximum it was fitted against. Once the rate rose past that maximum
        # the model saturated, and the card announced the rupee "strengthening"
        # by 9% because the model was pinned to a level from months earlier.
        #
        # A return is stationary, centred near zero, and bounded by what a
        # currency actually does in a day, so the same architecture has
        # something learnable to aim at. The rate is reconstructed from it in
        # predictor.py.
        feature_data = df[available_features].values
        close = df["close"].values.astype(float)
        # r_t = (close_{t+1} - close_t) / close_t, aligned so row t predicts the
        # move from t to t+1. The final row has no next day and is dropped.
        returns = np.diff(close) / close[:-1]
        feature_data = feature_data[:-1]
        target_data = returns.reshape(-1, 1)

        # Scale features.
        #
        # FIT ON THE TRAINING PORTION ONLY. This used to fit_transform the whole
        # array and split afterwards, so the MinMax range was taken from data
        # the model is then scored against. For USD/LKR that is not a subtle
        # leak: the series runs from ~200 to ~360 across the 2022 collapse, so
        # the scaler learned the eventual maximum before the model had "seen"
        # it, and the test scores flattered a model that could not really place
        # a level.
        split_row = int(len(feature_data) * 0.8)
        self.feature_scaler.fit(feature_data[:split_row])
        # StandardScaler for the target now that it is a return: MinMax would
        # again pin the model to the extremes of the training window, which is
        # the failure this replaced.
        self.target_scaler = StandardScaler()
        self.target_scaler.fit(target_data[:split_row])
    ...
    def train(
        ...
        try:
            ...
            #
            # In RUPEES, from the inverse-transformed arrays. Keras returns
            # test_mae on the SCALED target, and reporting that beside an
            # rmse in rupees produced "MAE: 0.0489 LKR, RMSE: 10.30 LKR" --
            # two numbers that cannot both describe the same errors, since RMSE
            # is never smaller than MAE. The scaled figure is kept but named
            # for what it is.
            rmse = np.sqrt(np.mean((y_pred - y_actual) ** 2))
            mae_lkr = float(np.mean(np.abs(y_pred - y_actual)))
            mape_lkr = float(
                np.mean(np.abs((y_pred - y_actual) / np.maximum(np.abs(y_actual), 1e-9)))
                * 100.0
            )
            ...
            results = {
                ...
                "test_mape_scaled": float(test_mape),
                "mae_return": mae_lkr,
                "mape_pct": mape_lkr,
                "rmse": float(rmse),
            ...
            logger.info(f"  MAE: {mae_lkr*100:.3f}% of rate  (scaled: {test_mae:.4f})")
            logger.info(f"  RMSE: {rmse*100:.3f}% of rate")
