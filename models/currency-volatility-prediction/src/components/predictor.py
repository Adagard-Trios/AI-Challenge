...
class CurrencyPredictor:
    ...
    def predict(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        # The model emits a next-day RETURN, so the rate is reconstructed from
        # today's close rather than read straight out. See the note on the
        # target in model_trainer.py: predicting the level meant the output was
        # capped at the training window's maximum, and once the rate passed it
        # the card reported a 9% move that was really the model saturating.
        current_rate = float(df["close"].iloc[-1])
        predicted_return = float(y[0, 0])
        predicted_rate = current_rate * (1.0 + predicted_return)
        change = predicted_rate - current_rate
        change_pct = predicted_return * 100.0
